    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('product.update', $product->id) }}">
                ...
                @for ($i = 1; $i <= 4; $i++)
                    <div class="form-group">
                        <label for="inputPhoto" class="col-form-label">Photo {{ $i }}<span
                                class="text-danger">*</span></label>
                        <div class="input-group">
                            <span class="input-group-btn">
                                <a id="pslfm{{ $i }}" data-input="thumbnail{{ $i }}"
                                    data-preview="holder{{ $i }}" class="lfm btn btn-secondary text-white">
                                    <i class="fa fa-picture-o"></i> Choose
                                </a>
                            </span>
                            <input id="thumbnail{{ $i }}" class="form-control" type="text"
                                name="photo{{ $i }}" value="{{ $product->{'photo' . $i} }}">
                        </div>
